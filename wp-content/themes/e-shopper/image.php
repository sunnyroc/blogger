							...
							<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
								<header class="entry-header">
									...
									<div class="entry-meta">
										...
										printf( __( 'Published', 'e-shopper' ) . ' <span class="entry-date"><time class="entry-date" datetime="%1$s">%2$s</time></span> - ' . __( 'size', 'e-shopper' ) . ': <a href="%3$s" title="Link to full-size image">%4$s &times; %5$s</a> ' . __( 'in', 'e-shopper' ) . ' <a href="%6$s" title="Return to %7$s" rel="gallery">%7$s</a>',
										...
										?>
										<?php edit_post_link( __( 'Edit', 'e-shopper' ), '<span class="sep"> | </span> <span class="edit-link">', '</span>' ); ?>
									</div>
									<!-- .entry-meta -->

									<nav id="image-navigation" class="row-fluid">
										<div class="col-md-6">
											<div class="previous-image"><?php previous_image_link( false, '&larr; ' . __( 'Previous', 'e-shopper' ) ); ?></div>
										</div>
										<div class="col-md-6">
											<div class="next-image alignright"><?php next_image_link( false, __( 'Next', 'e-shopper' ) . ' &rarr;' ); ?></div>
										</div>
									</nav>
									<!-- #image-navigation -->
								</header>
								...
								<div class="entry-content">
									...
									<?php the_content(); ?>
									<?php wp_link_pages( array( 'before' => '<div class="page-links">' . __( 'Pages:', 'e-shopper' ), 'after' => '</div>' ) ); ?>
								...
								<footer class="entry-meta">
									<?php if ( comments_open() && pings_open() ) : // Comments and trackbacks open ?>
										<?php printf( '<a class="comment-link" href="#respond" title="Post a comment">' . __( 'Post a comment', 'e-shopper' ) . '</a> ' . __( 'or leave a trackback', 'e-shopper' ) . ': <a class="trackback-link" href="%s" title="Trackback URL for your post" rel="trackback">' . __( 'Trackback URL', 'e-shopper' ) . '</a>.', get_trackback_url() ); ?>
									<?php elseif ( !comments_open() && pings_open() ) : // Only trackbacks open ?>
										<?php printf( __( 'Comments are closed, but you can leave a trackback:', 'e-shopper' ) . ' <a class="trackback-link" href="%s" title="Trackback URL for your post" rel="trackback">' . __( 'Trackback URL', 'e-shopper' ) . '</a>.', get_trackback_url() ); ?>
									<?php
									elseif ( comments_open() && !pings_open() ) : // Only comments open
										?>
										<?php _e( 'Trackbacks are closed, but you can', 'e-shopper' ) . ' <a class="comment-link" href="#respond" title="Post a comment">' . __( 'post a comment', 'e-shopper' ) . '</a>.'; ?>
									<?php
									elseif ( !comments_open() && !pings_open() ) : // Comments and trackbacks closed
										?>
										<?php _e( 'Both comments and trackbacks are currently closed.', 'e-shopper' ); ?>
									<?php endif; ?>
									<?php edit_post_link( __( 'Edit', 'e-shopper' ), ' <span class="edit-link">', '</span>' ); ?>
								</footer>
								<!-- .entry-meta -->
