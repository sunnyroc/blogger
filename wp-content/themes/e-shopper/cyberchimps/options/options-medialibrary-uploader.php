 ...
 */
if( is_admin() ) {
	// Load additional css and js for image uploads on the CyberChimps Framework Theme Options page
...
if( !function_exists( 'cyberchimps_medialibrary_uploader' ) ) {
	function cyberchimps_medialibrary_uploader( $_class, $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '' ) {
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button btn" type="button" value="' . __( 'Upload', 'e-shopper' ) . '" rel="' . $int . '" /></div>' . "\n";
...
if( !function_exists( 'cyberchimps_mlu_modify_tabs' ) ) {
	function cyberchimps_mlu_modify_tabs( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'e-shopper' ), __( 'Previously Uploaded', 'e-shopper' ), $tabs['gallery'] );
