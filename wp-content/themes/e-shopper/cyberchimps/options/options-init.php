...
function cyberchimps_admin_add_page() {
	$cyberchimps_page = add_theme_page(
		__( 'Theme Options Page', 'e-shopper' ),
		__( 'Theme Options', 'e-shopper' ),
		'edit_theme_options',
...
?>
	...
	<div class="row cc-header">
		<div class="col-md-4">
			<div class="cc-title">
				...
				<h2><?php printf( '%1s ' . __( 'Options', 'e-shopper' ), apply_filters( 'cyberchimps_current_theme_name', 'CyberChimps' ) ); ?></h2>
			</div>
			<!-- cc-title -->
		</div>
		<!-- col-md-4 -->
			...
			<div class="cc-submenu">
				<div class="cc-collapse">
					...
					<div class="cc-mobile-menu">
						<a class="btn btn-default" data-toggle="modal" href="#cc-mobile-modal">
				...
				<div class="cc-submenu-links">
					<input type="submit" id="cyberchimps_options_submit" class="btn btn-primary" name="update" value="<?php esc_attr_e( 'Save Options', 'e-shopper' ); ?>"/>
					<button class="reset-button btn" name="reset" onclick="return confirm( '<?php print esc_js( __( 'Click OK to reset. Any theme settings will be lost!',
					                                                                                                'e-shopper' ) ); ?>' );">
						<?php esc_attr_e( 'Restore Defaults', 'e-shopper' ); ?>
					</button>
				</div>
				<!-- cc-submenu-links -->
				<div class="clear"></div>
	...
	<div class="row">
		<div class="cc-footer">
			<div class="col-md-3">
				<div class="cc-logo">
					<a href="http://cyberchimps.com" title="<?php esc_attr_e( 'CyberChimps WordPress Themes', 'e-shopper' ); ?>"><img
							src="<?php echo get_template_directory_uri(); ?>/cyberchimps/options/lib/images/options/chimp.png"
							alt="<?php esc_attr_e( 'CyberChimps WordPress Themes', 'e-shopper' ); ?>"/><span>CyberChimps</span></a>
				</div>
				<!-- cc-logo -->
			</div>
			<!-- col-md-3 -->
...
function cyberchimps_create_fields( $fields ) {
	...
	foreach( $fields as $field_args ) {
		$field_defaults = array(
			'id'       => false,
			'name'     => __( 'Default Field', 'e-shopper' ),
...
function cyberchimps_fields_callback( $value ) {
	...
	switch( $value['type'] ) {
		...
		// Color picker
		case "color":
			$output .= '<div class="input-prepend ' . $value['class'] . '"><div id="' . esc_attr( $value['id'] . '_picker' ) . '" class="input-group-addon colorSelector"><div style="' . esc_attr( 'background-color:' . $val ) . '"></div></div>';
		...
		case 'typography':
			...
			if( $typography_options['color'] ) {
				$font_color = '<div class="input-prepend of-typography"><div id="' . esc_attr( $value['id'] ) . '_color_picker" class="input-group-addon colorSelector"><div style="' . esc_attr( 'background-color:' . $typography_stored['color'] ) . '"></div></div>';
		...
		case 'background':
			...
			// Background Color
			$output .= '<div class="input-prepend"><div id="' . esc_attr( $value['id'] ) . '_color_picker" class="input-group-addon colorSelector"><div style="' . esc_attr( 'background-color:' . $background['color'] ) . '"></div></div>';
...
function cyberchimps_options_validate( $input ) {
	...
	if( isset( $_FILES['import_file'] ) && $_FILES['import_file']['name'] ) {
		...
		if( $import_file_text ) {
			...
			if( $try ) {
				add_settings_error( 'cyberchimps_options', 'imported_success', __( 'Options Imported', 'e-shopper' ), 'updated fade' );

				return $try;
			}
			else {
				add_settings_error( 'cyberchimps_options', 'imported_failed', __( 'Invalid Data for Import', 'e-shopper' ), 'error fade' );
			}
		}
	}
	...
	if( isset( $_POST['reset'] ) ) {
		add_settings_error( 'cyberchimps_options', 'restore_defaults', __( 'Default options restored.', 'e-shopper' ), 'updated fade' );
	 ...
	 */
		...
		add_settings_error( 'cyberchimps_options', 'save_options', __( 'Options saved.', 'e-shopper' ), 'updated fade' );
...
function cyberchimps_admin_bar() {
	...
	$wp_admin_bar->add_menu( array(
		                         ...
		                         'title'  => __( 'Theme Options', 'e-shopper' ),
