...
//	wp_enqueue_style( 'bootstrap-responsive-style', $bootstrap_path . 'css/bootstrap-responsive.min.css', 'bootstrap-style' , '3.3.4' );
	wp_enqueue_script( 'bootstrap-js', $bootstrap_path . 'js/bootstrap.min.js', array( 'jquery' ), '3.3.4', true );

	//responsive design
	if( cyberchimps_get_option( 'responsive_design', 'checked' ) ) {
		wp_enqueue_style( 'cyberchimps_responsive', $directory_uri . '/cyberchimps/lib/bootstrap/css/cyberchimps-responsive.min.css', 'bootstrap-style', '1.0' );
	}
	else {
...
//add meta entry category to single post, archive and blog list if set in options
	function cyberchimps_posted_in() {
		...
		if( $show ):
			...
			if( $categories_list ) :
				$cats = sprintf( __( 'Posted in %s', 'e-shopper' ), $categories_list );
				?>
...
//add meta entry comments to single post, archive and blog list if set in options
	function cyberchimps_post_comments() {
		...
		$leave_comment = ( is_single() || is_page() ) ? '' : __( 'Leave a comment', 'e-shopper' );
		if( $show ):
			if( !post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) : ?>
				<span class="comments-link"><?php comments_popup_link( $leave_comment, __( '1 Comment', 'e-shopper' ), '% ' . __( 'Comments', 'e-shopper' ) ); ?></span>
...
function cyberchimps_comment_form_filter( $defaults ) {
	$defaults['title_reply'] = __( 'Leave a comment', 'e-shopper' );
	...
	if( $show ):
		if( has_post_thumbnail() ): ?>
			<div class="featured-image">
				<a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'e-shopper' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark">
...
function cyberchimps_default_site_title() {
	...
	if( is_archive() ) {
		echo ' | ';
		if( is_category() ) {
			printf( __( 'Category Archives: %s', 'e-shopper' ), single_cat_title( '', false ) );
		}
		elseif( is_tag() ) {
			printf( __( 'Tag Archives: %s', 'e-shopper' ) , single_tag_title( '', false ) );
		}
		elseif( is_author() ) {
			_e( 'Author Archives', 'e-shopper' );
		}
		elseif( is_day() ) {
			printf( __( 'Daily Archives: %s', 'e-shopper' ), get_the_date() );
		}
		elseif( is_month() ) {
			printf( __( 'Monthly Archives: %s', 'e-shopper' ), get_the_date( 'F Y' ) );
		}
		elseif( is_year() ) {
			printf( __( 'Yearly Archives: %s', 'e-shopper' ), get_the_date( 'Y' ) );
		}
		elseif( is_plugin_active( 'woocommerce/woocommerce.php' ) && is_woocommerce() && is_shop() ) {
			_e( 'Shop', 'e-shopper' );
		}
		else {
			_e( 'Archives', 'e-shopper' );
		}
	}

	//Title for search
	if( is_search() ) {
		echo ' | ' . sprintf( __( 'Search for &quot; %s &quot;', 'e-shopper' ), get_search_query() );
	}

	//Title for 404
	if( is_404() ) {
		echo ' | ' . __( 'Not Found', 'e-shopper' ) . ' ';
	}
	...
	if( $paged >= 2 || $page >= 2 ) {
		echo ' | ' . sprintf( __( 'Page %s', 'e-shopper' ), max( $paged, $page ) );
	}
}
...
function cyberchimps_search_excerpt_more( $more ) {
	...
	if( cyberchimps_get_option( 'search_post_read_more' ) != '' ) {
		...
		$more = '<p><a class="excerpt-more search-excerpt" href="' . get_permalink( $post->ID ) . '">' . __( 'Read More...', 'e-shopper' ) . '</a></p>';
...
function cyberchimps_options_help_text() {
	...
	$text .= '<div class="cc_help_section">
						<div class="row"><div class="col-md-3">
							<a href="' . apply_filters( 'cyberchimps_documentation', 'http://cyberchimps.com' ) . '" title="CyberChimps Instructions">
								...
								<div class="cc_help_caption"><p>' . __( 'Instructions', 'e-shopper' ) . '</p></div>
							</a>
						</div>
						<div class="col-md-3">
							<a href="' . apply_filters( 'cyberchimps_support_forum', 'http://cyberchimps.com' ) . '" title="CyberChimps Support">
								...
								<div class="cc_help_caption"><p>' . __( 'Support', 'e-shopper' ) . '</p></div>
							</a>
						</div>
						</div>';
	...
	if( cyberchimps_theme_check() == 'free' ) {
		...
		$text .= '<div class="row">
						...
						<div class="cc_help_upgrade_bar">' . sprintf( __( 'Upgrade to %1$s', 'e-shopper' ), $pro_title ) . '</div>
						</a>
						</div>
						</div>
						</div>
						<div class="clear"></div>';
		$text .= sprintf(
			'<p>' . __( 'For even more amazing new features, upgrade to %1$s which includes a slider with customizable features, an image carousel, widgetized boxes, a callout section, expanded typography (including Google Fonts), additional color skins, and many more powerful new features. Visit %2$s to learn more!', 'e-shopper' ) . '</p>',
...
function cyberchimps_upgrade_bar() {
	...
	<div class="upgrade-callout">
		<p><img src="<?php echo get_template_directory_uri(); ?>/cyberchimps/options/lib/images/chimp.png" alt="CyberChimps"/>
			<?php printf(
				__( 'Welcome to %1$s! Learn more now about upgrading to %2$s today.', 'e-shopper' ),
				$current_theme_name,
...
<p>
<?php 
	$utm_link="https://cyberchimps.com/free-download-50-stock-images-use-please/?utm_source=e-Shopper";
...
function cyberchimps_admin_link() {
	...
	$wp_admin_bar->add_menu( array(
		                         'id'    => 'cyberchimps',
		                         'title' => apply_filters( 'cyberchimps_current_theme_name', 'CyberChimps ' . __( 'Options', 'e-shopper' ) ) . __( 'Options', 'e-shopper' ),
...
function cyberchimps_addon_sections( $sections_list ) {
	...
	'label'   => __( 'SlideDeck Lite', 'e-shopper' ),
	'heading' => 'cyberchimps_addons_heading'
	);

	$sections_list[] = array(
	'id'      => 'cyberchimps_wp_forms_options',
	'label'   => __( 'WP Forms', 'e-shopper' ),
	'heading' => 'cyberchimps_addons_heading'
	);
...
function cyberchimps_addon_fields( $fields_list ) {
	...
	$fields_list[] = array(
		'name'     => __( 'SlideDeck Lite', 'e-shopper' ),
		'id'       => 'slidedeck_lite',
		'type'     => 'info',
	...
	$fields_list[] = array(
	'name'     => __( 'WP Forms', 'e-shopper' ),
	'id'       => 'wp_forms',
	'type'     => 'info',
	'callback' => 'cyberchimps_wp_forms_callback',
	'section'  => 'cyberchimps_wp_forms_options',
	'heading'  => 'cyberchimps_addons_heading'
	);
...
function cyberchimps_slidedeck_lite_callback( $value ) {
	...
	if( isset( $installed_plugins[$plugin] ) ) {
		if( is_plugin_active( $plugin ) ) {
			$output .= $icon . '<a href="' . admin_url( 'admin.php?page=slidedeck.php' ) . '">' . __( 'SlideDeck Settings', 'e-shopper' ) . '</a>';
		}
		else {
			$output .= $icon_neg . '<a href="' . admin_url( 'plugins.php' ) . '">' . __( 'Please activate the "SlideDeck Lite" plugin', 'e-shopper' ) . '</a>';
		}
	}
	else {
		$output .= $icon_neg . '<a href="' . cyberchimps_slidedeck_install_link() . '">' . __( 'Install the "SlideDeck Lite" plugin', 'e-shopper' ) . '</a>';
	}
...
// WP Forms Text
function cyberchimps_wp_forms_callback( $value ) {
	$output   = '';
	...
	if( isset( $installed_plugins[$plugin] ) ) {
		if( is_plugin_active( $plugin ) ) {
			$output .= $icon . '<a href="' . admin_url( 'admin.php?page=wpforms-overview' ) . '">' . __( 'WP Forms Settings', 'e-shopper' ) . '</a>';
		}
		else {
			$output .= $icon_neg . '<a href="' . admin_url( 'plugins.php' ) . '">' . __( 'Please activate the "WP Forms Lite" plugin', 'e-shopper' ) . '</a>';
		}
	}
	else {
		$output .= $icon_neg . '<a href="' . cyberchimps_wpforms_install_link() . '">' . __( 'Install the "WP Forms Lite" plugin', 'e-shopper' ) . '</a>';
	}
